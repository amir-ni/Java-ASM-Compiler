...
public class slt extends Instruction { // slt $rd, $rs, $rt
    private static int OPCODE = 0;
    private static int FUNCT = 40;

    public slt(Assembler assembler, String instruction, String instructionPart2, int startClock, long address) throws UnformattedInstructionException, IllegalRegisterNumberException {
    ...
    public void run() {
        ...
        if (registers[getRs_register()] < registers[getRt_register()])
            registers[getRd_register()] = 1;
        else
            registers[getRd_register()] = 0;
    }
